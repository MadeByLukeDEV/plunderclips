-- Initial migration for SoT Clips
-- Run with: npm run db:migrate (uses Prisma migration system)
...
CREATE TABLE IF NOT EXISTS clips (
  ...
  CONSTRAINT clips_submitted_by_fkey FOREIGN KEY (submitted_by) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS clip_tags (
  id VARCHAR(191) NOT NULL,
  clip_id VARCHAR(191) NOT NULL,
  tag ENUM('FUNNY','KILL','TUCK','HIGHLIGHT','PVP','PVE','SAILING','TREASURE','KRAKEN','EPIC_FAIL','TEAM_PLAY','SOLO') NOT NULL,
  PRIMARY KEY (id),
  UNIQUE KEY clip_tags_clip_id_tag_key (clip_id, tag),
  CONSTRAINT clip_tags_clip_id_fkey FOREIGN KEY (clip_id) REFERENCES clips(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
