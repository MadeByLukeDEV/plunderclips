...
import { Shield, CheckCircle, XCircle, Clock, Users, Film, AlertTriangle } from 'lucide-react';
            ...
            {clipsData?.clips?.map((clip: any) => (
              <div key={clip.id} className="sot-card rounded flex items-start gap-4 p-3 hover:border-teal/20 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Show review notes on declined tab */}
                  {tab === 'DECLINED' && clip.reviewNotes && (
                    <p className="text-xs text-red-400/60 font-body mt-1 flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3 flex-shrink-0" />{clip.reviewNotes}
                    </p>
                  )}
                </div>
                {tab === 'PENDING' && (
                  <div className="flex gap-2 flex-shrink-0">
                    <button
                      onClick={() => mutation.mutate({ id: clip.id, status: 'APPROVED' })}
